...
class Pager
{
     ...
     */
    public function __construct($data = array(), $model = '')
    {
        $this->total_rows = $data['total_rows'];
        $this->parameter = !empty($data['parameter']) ? $data['parameter'] : '';
        $this->list_rows = PAGESIZE;//!empty($data['list_rows']) && $data['list_rows'] <= 100 ? $data['list_rows']: 3;
    ...
    protected function _get_url($page)
    {
        ...
        if(!empty($this->model)){
            $url = CreateUrl::createSearchUrl($this->model, array('page' => $page));
            return $url;
        }else{
           // return $this->url.$this->page_name.'='.$page;
            $url = explode('/', $this->url);
            $url[3] = $page;
            return $this->url = implode($url, '/');
        }
    }
    ...
    protected function show_2()
    {
        if ($this->total_pages != 1) {
            ...
            for ($i = 1; $i <= $this->total_pages; $i++) {
                if ($i == $this->now_page) {
                    $return .= "<li><a href='' class='active'>$i</a></li>";
                } else {
                    if ($this->now_page - $i >= 3 && $i != 1) {//4换成了３
                        $return .= "<li>...</li>";
                        $i = $this->now_page - 3;
                    } else {
                        if ($i >= $this->now_page + 4 && $i != $this->total_pages) { //５换成了３
                            $return .= "<li>...</li>";
                            $i = $this->total_pages;
                        }
